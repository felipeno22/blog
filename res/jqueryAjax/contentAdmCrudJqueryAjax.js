...
function contents(search,dtiniinfo,dtfinalinfo,pag,aux,aux2){
  ...
  $.ajax({
     ...
     $("tbody tr").remove();
      ...
      for(var i=0; i < result['contents'].length; i++){
        ...
        $("tbody").append('<tr><td hidden>'+result['contents'][i].idcontent+'</td><td>'+result['contents'][i].title+'</td><td>'+result['contents'][i].dtcontent+'</td><td hidden>'+result['contents'][i].idblog+'</td><td><a  id="updateLink" href="/updateCont/'+result['contents'][i].idcontent+'" class="btn btn-primary btn-xs"><i class="fa fa-edit"></i> Editar</a><a  id="deleteLink"  href="/content/delete/'+result['contents'][i].idcontent+'" onclick="return confirm(\'Deseja realmente excluir este registro?\')" class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> Excluir</a></tr>');
...
function page(e,aux,aux2){
  ...
  var dtfinalinfo=$("#dtfinalinfo").val();
            ...
            informatives(search,dtiniinfo,dtfinalinfo,e,aux,aux2)
